...
def plot_example(ax, SNR_target, sim_time, x,  data, type):
    meanRate=round(SNR_target.signals['spikes'].mean_rate(1000,sim_time),1)
    spk=SNR_target.signals['spikes'].time_slice(1000,sim_time).raw_data()
    ...
    ax.set_title( type+' mean='+str(round(data[0],3))+ 'Hz, std='+str(round(data[1],3))+
                  'Hz, CV='+str(round(CV,3))+
                 ',\n n_gpe_snr='+str(round(x[0],3))+', snr_curr='+str(round(x[1],3))+ 
                  ' w_STN_SNR='+str(round(x[2],3)) )

        
        
def restriction_1(msn_rate, gpe_rate, stn_rate_ch, n_msn, n_gpe, x,  neuron_model, syn_models, n_exp):
...
#    to come close to restriction STN needs to be depressing onto SNr  
    ...
    SNR_list=simulate_basa_line_SNr_multiple(msn_rate, gpe_rate, stn_rate_ch, 
                                             ...
                                             record_vm=True, n_neurons=n_exp) 
...
def restriction_2(msn_rate, gpe_rate, stn_rate, n_msn, n_gpe, x, neuron_model, syn_models, n_exp):
        # 2. 15 Hz without STN. OBS Since stn is of the GPE should fire at 15 Hz
        c, w=x
        target_rate2=15.0
        stn_rate_ch=0.
...
def restriction_3(msn_rate, gpe_rate, stn_rate, n_msn, n_gpe, x, neuron_model, syn_models, n_exp):
        # 3. 30 Hz normal, GPe at 30 Hz, STN at 10 Hz
        c, w=x
        target_rate3=30.0
...
def error_fun(x, sim_time,n_exp, n_gpe, res1_stn_rate_ch):
        ...
        SNR_target,mr, std, e1=restriction_1(msn_rate, gpe_rate, res1_stn_rate_ch, n_msn, n_gpe, x, neuron_model, syn_models, n_exp)
        SNR_target,mr, std, e2=restriction_2(msn_rate, gpe_rate, stn_rate, n_msn, n_gpe, x, neuron_model, syn_models, n_exp)
        SNR_target,mr, std, e3=restriction_3(msn_rate, gpe_rate, stn_rate, n_msn, n_gpe, x, neuron_model, syn_models, n_exp)
...
def fmin(load, save_at,  x0, n_exp, n_gpe, res1_stn_rate_ch):
    ...
    if not load:
        [xopt,fopt, iter, funcalls , warnflag, allvecs] = opt.fmin(error_fun, x0, 
                                                                   args=([sim_time, n_exp, n_gpe,res1_stn_rate_ch]), maxiter=50, maxfun=10,full_output=1, retall=1)
...
#    x0=[220, 1.]
#    x=fmin(0,save_at, x0, n_exp, n,  res1_stn_rate_ch)
#    
#res1_stn_rate_ch=20
#save_at=OUTPUT_PATH+'/simulate_network_fmin.plk' 
#n_conn=[22,25]
...
SNR_target1, mr1, std1,  e1=restriction_1(msn_rate, gpe_rate, stn_rate, n_msn, x, neuron_model, syn_models, n_exp)
SNR_target2, mr2, std2,  e2=restriction_2(msn_rate, gpe_rate, stn_rate, n_msn, x, neuron_model, syn_models, n_exp)
SNR_target3, mr3, std3,  e3=restriction_3(msn_rate, gpe_rate, stn_rate, n_msn, x, neuron_model, syn_models, n_exp)
