...
SYNAPSE_MODELS=['MSN_SNR_gaba_s_min', 'MSN_SNR_gaba_s_max', 
                'GPE_SNR_gaba_s_ref','STN_SNR_ampa_s']

# Misc
SNR_INJECTED_CURRENT=-26.
...
def plot_response_example_clamped_silent(ax, SNR):
    ...
    #             fontsize=pylab.rcParams['']-4, 
    #             **{'color': 'k'})
        
    ax.set_xlim(misc.adjust_limit([0,300]))
    ax.set_ylim(misc.adjust_limit([-67,-55]))
...
def plot_voltage_ipsp(ax, v_mat, ipsp_mat):
    colors = ['b', 'g','r']
    labels=[r'$ref_{init}^{MSN}$' , r'$ref_{max}^{MSN}$',  r'$ref_{32 Hz}^{GPe}$']
    ...
    for label, coord, color in zip(labels,coords,colors):
        ax.text( coord[0], coord[1], label, transform=ax.transAxes, 
                 fontsize=pylab.rcParams['']+2, **{'color': color}) 
