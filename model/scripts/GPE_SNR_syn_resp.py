...
def plot_response_example_clamped_silent(ax, SNR):
    ...
    ax.text( 0.7, 0.4,'Weak' , transform=ax.transAxes, **{ 'color' : 'g' }) 
    ax.text( 0.7, 0.3, 'Strong' , transform=ax.transAxes, **{ 'color' : 'r' }) 
...
def plot_voltage_ipsc(ax, voltage, ipsc):
    ...
    ax.text( 0.1, 0.4,'Weak' , transform=ax.transAxes, **{ 'color' : 'g' }) 
    ax.text( 0.2, 0.8, 'Strong' , transform=ax.transAxes, **{ 'color' : 'r' }) 

def plot_voltage_ipsp(ax, voltage, ipsp):
    ax.plot(voltage,ipsp[0,:],**{'color':'g'})
    ax.plot(voltage,ipsp[1,:],**{'color':'r'})
    ax.my_set_no_ticks( yticks=4, xticks = 4 ) 
...
def simulate_voltage_ipsc(I_vec):
    ...
    SNR = MyGroup( NEURON_MODELS[0], len(SYNAPES_MODELS), mm_dt = 0.1 )
...
voltage, ipsc=simulate_voltage_ipsc(range(-300,-130,10))

# Voltage against ipsp size
voltage, ipsp=simulate_voltage_ipsp(range(-300,-130,10))
