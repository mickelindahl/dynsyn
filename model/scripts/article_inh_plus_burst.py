...
def plot_example_raster_GPE(ax, GPE_list):
    ...
    ax.text( 0.05, 0.35, 'GPe neurons with a pause'  , backgroundcolor='w',
             transform=ax.transAxes, **{ 'color' : 'r' })  
    ax.text( 0.05, 0.25, 'plus a burst' , backgroundcolor='w',
             transform=ax.transAxes, **{ 'color' : 'r' })     
    ax.text( 0.05, 0.15, 'GPe neurons with no rate'  , backgroundcolor='w',
             transform=ax.transAxes, **{ 'color' : 'b' })  
    ax.text( 0.05, 0.05, 'change' , backgroundcolor='w',
             transform=ax.transAxes, **{ 'color' : 'b' }) 
...
def plot_example_firing_frequency_GPE(ax, GPE_list):
    ...
    ax.set_xlim(misc.adjust_limit([0,1700]))
    ax.set_ylim(misc.adjust_limit([0,170]))
    ...
    misc.slice_line(lines[0], xlim=[0,1690])
    misc.slice_line(lines[1], xlim=[0,1690])
    misc.slice_line(lines[2], xlim=[0,1690])
    
    #ax.set_title('bin=%i'%(time_bin),**{'fontsize':12})
    ax.set_ylabel('Firing rate GPe (spikes/s)')     
    ax.my_set_no_ticks( yticks=6, xticks = 5 )

    ax.text( 0.05, 0.85, 'GPe neurons with a pause' , backgroundcolor='w',
             transform=ax.transAxes, **{ 'color' : 'r' }) 
    ax.text( 0.05, 0.75, 'plus a burst' , backgroundcolor='w',
             transform=ax.transAxes, **{ 'color' : 'r' }) 
    ax.text( 0.05, 0.65, 'GPe neurons with no rate' , backgroundcolor='w',
             transform=ax.transAxes, **{ 'color' : 'b' })  
    ax.text( 0.05, 0.55, 'change' , backgroundcolor='w',
             transform=ax.transAxes, **{ 'color' : 'b' })  
    ax.text( 0.05, 0.45, 'Average over all' , backgroundcolor='w',
...
def simulate_example(hz_1=0.,hz_2=100., load=True):
    ...
    if not load:
        for i_exp in EXPERIMENTS:    
            ...
            for id in GPE[N_GPE-N_SEL:N_GPE]: 
                ...
                times = [1, SEL_ONSET, SEL_ONSET+SEL_TIME_1,
    ...
    s = s + ' %s %5s %3s \n' % ( 'Selection rate:', str ( RATE_SELE_1 ), 'spikes/s' )
    s = s + ' %s %5s %3s \n' % ( 'Selection time:', str ( SEL_TIME_1 ), 'ms' )
    s = s + ' %s %5s %3s \n' % ( 'Selection rate:', str ( RATE_SELE_2 ), 'spikes/s' )
...
if not mpiRun:
    ...
    GPE_list, SNR_list, s=simulate_example(hz_2=75, load=False)
