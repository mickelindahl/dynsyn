...
SYNAPES_MODELS=['GPE_SNR_gaba_s_min', 'GPE_SNR_gaba_s_max']

def plot_response_example_clamped_silent(ax, SNR):
    colors = ['g', 'y']
    SNR.signals['V_m'].plot(id_list=[SNR[0]], display=ax,kwargs={'color':colors[0]})
    #SNR.signals['V_m'].plot(id_list=[SNR[1]], display=ax,kwargs={'color':colors[1]})
    ax.text( 0.7, 0.4,r'$\delta_r^{GPe}$' , fontsize=pylab.rcParams['text.fontsize'],
             transform=ax.transAxes, **{ 'color' : colors[0] }) 
    #ax.text( 0.7, 0.3, 'Strong' , transform=ax.transAxes, **{ 'color' : colors[1] }) 
...
def plot_response_example_clamped_spiking(ax, SNR):
    ...
    ax.my_set_no_ticks( yticks=4, xticks = 4 )   
    ax.text( 0.1, 0.6,'Weak' , transform=ax.transAxes, **{ 'color' : 'g' }) 
    ax.text( 0.1, 0.5, 'Strong' , transform=ax.transAxes, **{ 'color' : 'r' }) 
...
def plot_voltage_ipsc(ax, voltage, ipsc):
    ...
    ax.text( 0.1, 0.4,'Ref' , transform=ax.transAxes, **{ 'color' : 'g' }) 
    #ax.text( 0.2, 0.8, 'Strong' , transform=ax.transAxes, **{ 'color' : 'r' }) 
...
def plot_voltage_ipsp(ax, voltage, ipsp):
    ...
    ax.set_xlabel('Holding potential (mV)')
    ax.set_ylim()
    ax.text( 0.1, 0.55,'Ref' , transform=ax.transAxes, **{ 'color' : colors[0] }) 
    #ax.text( 0.35, 0.8, 'Strong' , transform=ax.transAxes, **{ 'color' : colors[1] }) 
...
def plot_text(ax, infoString):
    ...
    tb = tb + '\n'
    tb = tb + ' %s %5s %3s \n' % ( 'E rev:', 
                                   str( statusSNR['GABAA_2_E_rev' ] ), 
                                   'mV' )
    tb = tb + ' %s %5s %3s \n' % ( 'Tau_decay:', 
                                   str( statusSNR['GABAA_2_Tau_decay' ] ), 
                                   'mV' )  
...
def simulate_response_example_clamped_silent(I_e):
    ...
    s = s + ' %s %5s %3s \n' % ( 'Size weak:',   str ( round(size_weak,1) ),   'mV' )     
    s = s + ' %s %5s %3s \n' % ( 'Size strong:', str ( round(size_strong,1) ), 'mV' )
...
def simulate_voltage_ipsc(I_vec):
    ...
    SNR.get_signal( 'c','I_GABAA_2', stop=simTimeTot ) # retrieve signal
    simTimeAcum=0
    
    
    for I_e in I_vec:
        
        signal=SNR.signals['I_GABAA_2'].my_time_slice(400+simTimeAcum, 700+simTimeAcum)
        simTimeAcum+=simTime
...
SNR_plot2 = simulate_response_example_clamped_spiking(80.)
