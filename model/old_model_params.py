...
def models( Params_in = {} ):
    ...
    CTX_STN_g_ampa =  2.5                                                       # (Baufreton et al. 2005)
    CTX_STN_g_nmda =  0.07                                                      # n.d.; same ratio ampa/nmda as MSN
    GPE_STN_g_gaba = .1                                                         # (Baufreton et al. 2005)

    MSN_GPE_g_gaba = 2.                                                         # Half of  (Sims et al. 2008)
    STN_GPE_g_ampa =  0.5                                                       # (Hanson & Dieter Jaeger 2002)
    STN_GPE_g_nmda =  0.01                                                      # n.d; estimated
    GPE_GPE_g_gaba = 2.                                                         # (Sims et al. 2008)

    MSN_SNR_g_gaba = [2., 23.5]                                                 # Lower based on (Connelly et al. 2010) = [4.7, 24.], 50 Hz model = [5.8, 23.5]
    STN_SNR_g_ampa =  0.5                                                       # (Shen and Johnson 2006)
    STN_SNR_g_nmda =  0.01                                                      # n.d.; same ratio ampa/nmda as for MSN
    #GPE_SNR_g_gaba = [16., 70.]                                                # (Connelly et al. 2010)
    GPE_SNR_g_gaba = [0.17*76., 76.]                                            # (Connelly et al. 2010)
    ...
    #! STN 75 percent
    #! --------------
    # Non-linear I-F curve and HFSN AHP
    ''' 
    a      = [ 0.02, 0.002]                                                     # I-F relation
    b_1    = [ 5., 0.   ]                                                       # I-V relation, rebound burst
    b_2    = [ 0.3, 0.   ]                                                      # I-F relation
    c      = -60.                                                               # Beurrier et al. (1999)
    C      =  27.                                                               # t_m/R_in    
    d      = [ 15., 0.1 ]                                                       # I-F relation, AHP 
    k      =   0.09                                                             # I-V relation
    k_c    =  -0.15                                                             # Accelerating I-F relation
    v_b    = -70.                                                               # I-V relation, (Bevan et al. 2000; Bevan & C J Wilson 1999; Nicholas E Hallworth et al. 2003) 
    v_peak =  31.                                                               # (Cooper & I M Stanford 2000)
    v_r    = -64.                                                               # (Kass & Mintz 2006)
    v_t    = -35.                                                               # (M. A. Farries et al. 2010)    
    t_m    =   5.3                                                              # (Paz et al. 2005)
    R_in   = 195.                                                               # (Beurrier et al. 1999)
    
    params = { 'c' : c, 'C_m' : C, 'k' :  k, 'kc_2':   k_c, 'V_peak' : v_peak,
             'V_r' : v_r, 'V_t' : v_t, 'u_kc': 0.0, 'I_e' : 26.5, 'V_m' :-60.0,
             'recovery_0':
             { 'a' : a[0], 'b_1' : b_1[0], 'b_2' : b_2[0], 'd_1' : d[0],
               'd_2' : d[0], 'd_1_bound' : 0.0, 'V_b' : v_b, 'u' : 0.0,
               'u_const' : 0., 'u_max' : 60.0},
             'recovery_1':
             { 'a' : a[1], 'b_1' : b_1[1], 'b_2' : b_2[1], 'd_1' : d[1],
               'd_2' : d[1], 'd_1_bound' : 0.0, 'V_b' : v_b, 'u' : 0.0,
               'u_const' : 0., 'u_max' : 60.0},
    ...
    '''
    a      =   0.005                                                            # I-F relation
    b_1    =   1.                                                               # I-V relation, rebound burst
    b_2    =   0.                                                               # I-F relation
    c      = -65.                                                               # Beurrier et al. (1999)
    c_max  = -40.
    C      =  27.                                                               # t_m/R_in    
    d      =   3.                                                               # I-F relation, AHP 
    k      =   0.09                                                             # I-V relation
    kc_1   =  -0.5                                                              # Accelerating I-F relation
    kc_2   =   0.3                                                              # Accelerating I-F relation
    u_kc   =   0. 
    v_b    = -70.                                                               # I-V relation, (Bevan et al. 2000; Bevan & C J Wilson 1999; Nicholas E Hallworth et al. 2003) 
    v_peak =  31.                                                               # (Cooper & I M Stanford 2000)
    v_r    = -64.                                                               # (Kass & Mintz 2006)
    ...
    # Linear I-F
    params={ 'a' : a, 'b_1' : b_1, 'b_2' : b_2, 'c' : c, 'c_max': c_max, 'C_m' :  C, 
            'd' : d, 'k' : k, 'kc_1' : kc_1, 'kc_2' : kc_2, 'p_2' :  1., 
            'V_b' : v_b, 'V_peak':  v_peak, 'V_r' : v_r, 'V_t' : v_t, 
            'u_kc' : u_kc, 'u_max' : u_max, 'I_e' :  8., 'V_m'   :-60., 
            'AMPA_Tau_decay'   : CTX_STN_tau_ampa,
            'NMDA_Tau_decay'   : CTX_STN_tau_nmda,
            'NMDA_Vact'        :-20.0,
            'NMDA_Sact'        : 16.0,
            'GABAA_1_Tau_decay': GPE_STN_tau_gaba,
            'GABAA_1_E_rev'    : GPE_STN_E_gaba, } 
    model_list.append(('izhik_cond_exp', 'STN_75', params))
    ...
    # Reverse spike time adaptation
    params = { 'C_m' :   25., 'c'  :-56., 'k'       :  0.09,
             'V_r' :-62., 'V_t' :-42., 'V_peak'  : 15.,
             'I_e' :  30.0, 'V_m' :-60.0, # Gives 10 hz firing rate 
            
             'recovery_0':
             { 'V_b' :-67., 'a'       :  0.03,
               'b_1' :    0., 'b_2'     :  0.00075,
               'd_1' :    200., 'd_2'     :  200., 'd_1_bound' : 0.0,
               'u'   :   0.0, 'u_const' : 0., 'p_2'       :  3., },
             
             'recovery_1':
             { 'V_b' :    0., 'a'       :  0.0067,
               'b_1' :    0., 'b_2'     :  0.,
               'd_1' :-1., 'd_2'     :-1., 'd_1_bound' : 0.0,
               'u'   :   0.0, 'u_const' : 0., },
            ...
            'NMDA_Vact'        :-20.0, 'NMDA_Sact'     : 16.0, # Humphrie 2009
            'GABAA_1_Tau_decay': 10.0, 'GABAA_1_E_rev' :-84.0, }              
    model_list.append(('izhik_cond_exp_mu' , 'STN_75_RSA' , params))
    ...
    a      =  0.2                                                               # I-F relation, spike frequency adaptation
    b_1    =  0.                                                                # I-V relation
    b_2    =  0.0001                                                            # I-V relation
    c      = -55.                                                               # I-F relation, spike form
    C      =  40.#34.#25.                                                           # t_m/R_in
    d      =  0.                                                                # I-F relation
    k      =   0.05                                                             # I-V relation
    k_c    =  0.#-0.1                                                             # I-V relation
    p_1    =   1.                                                               # I-V relation
    p_2    =   3.                                                               # I-F relation
    v_b    = -55.                                                               # (Bugaysen et al. 2010; Cooper and I M Stanford 2000)
    v_peak =  0. #31.                                                           # (Cooper and I M Stanford 2000)
    v_r    = -54.                                                               # (Bugaysen et al. 2010)
    v_t    = -43.                                                               # (Bugaysen et al. 2010)
    t_m    =  24.4                                                              # (Cooper and I M Stanford 2000)
    R_in   = 558.9                                                              # (Cooper and I M Stanford 2000)
    ...
    GPE_GPE_E_gaba    = -65.                                                    # (Rav-Acha et al. 2005)

    
    
    #! GPE slow:
    #! ------------------------------
    #! Type A (18%) + C (50%) Bugaysen 2010 and Type A (63%) Cooper 2000
    
    params = { 'a' : a, 'b_1' :  b_1, 'b_2'  : b_2, 'c' : c, 'C_m' : C,
               'd' : d, 'k' :  k, 'kc_1' : k_c, 'kc_2' : k_c,  'p_2'  :  p_2,
               'V_b' : v_b, 'V_peak' : v_peak, 'V_r' : v_r, 'V_t' : v_t,
               'I_e' : 10., 'u_const' :  0., 'V_m' :-60.,     
    ...
    model_list.append(('izhik_cond_exp' , 'GPE_A_C' , params))
    
    
    #! GPE Type B ( 24/(14+38+24)=32 % Bugaysen,  2010 (se matlab)
    #! -----------
    #! (32 %) Type 2 behavior with oscillation at rest Terman 2002, org ref 
    #! Nambu 1994. Fast a (0.02) gives type 1 behavior subthreshold 
    #! oscillations. Also exhibit short rebound burst  
    params = params.copy()                                                      # Copy GPe A parameters
    params.update({ 'k'   : 0.07, 'V_r' :-58., 'V_t'    :-43.,
                     'C_m' : 34.0, 'c'   :-54., 'V_peak' :  27.0,
                     'd' : 2., 'I_e'  :  10., })
    model_list.append(('izhik_cond_exp', 'GPE_B', params))
    ...
    #! Izhikevich 
#    a      =   0.02                                                            # I-V relation, spike frequency adaptation
#    b_1    =   2.                                                           # I-V relation
#    b_2    =   2.                                                           # I-F relation
#    c      = -60.                                                               # I-V relation
#    C      =  100.                                                               # t_m/R_in
...
#    p_1    =   1.                                                               # I-V relation
#    p_2    =   1.                                                               # I-F relation
#    v_b    = -62.5                                                               # I-V relation
#    v_peak =  10.                                                               # Richards 1997
#    v_r    = -62.5                                                               #
#    v_t    = -62.5                                                               # 
#    t_m    =   8.                                                               # Nakanishi et al. (1997)
#    R_in   =  97.3                                                              # Nakanishi et al. (1997)
#    I_e   = 95.

    a      =   0.05          # I-V relation, spike frequency adaptation
    b_1    =   4.           # I-V relation
    b_2    =   0.01       # I-F relation
    c      = -65.           # I-V relation
    C      =  80.           # t_m/R_in
    d      =   0.           # I-F relation
    k      =   0.5          # I-V relation
    ...
    I_e   = 100.
#    a      =   0.02 # I-V relation, spike frequency adaptation
#    b_1    =   0.0 # I-V relation
#    b_2    =   0.0007 # I-F relation
#    c      = -65. # I-V relation
#    C      =  80. # t_m/R_in
#    d      =  0.0 # I-F relation
#    k      =  0.2 # I-V relation
...
#    I_e   = 100.
    ...
    params = { 'a' : a, 'b_1' : b_1, 'b_2' : b_2, 'c' : c, 'C_m' : C,
               'd' : d,  'k' : k, 'p_2' : p_2, 'V_b' : v_b,
               'V_peak' : v_peak, 'V_r' : v_r, 'V_t' : v_t, 'I_e' : I_e,
    ...
    model_list.append(('izhik_cond_exp', 'SNR_izh', params))
    ...
    #! Alt 2 
#    a      =   0.06                                                            # I-V relation, spike frequency adaptation
#    b_1    =   1.322                                                            # I-V relation
#    b_2    =   1.322                                                            # I-F relation
#    c      = -59.                                                               # I-V relation
#    C      =  120.                                                               # t_m/R_in
#    d      =  100.                                                            # I-F relation
#    gleak  =   5.
...
#  
    ...
    params = { 'C_m':C, 'E_L' : v_r, 'g_L' : gleak,
               'V_th':v_peak, 'V_reset':c,  'I_e' : 126.092+60,
               'AMPA_Tau_decay'    : STN_SNR_tau_ampa,
               'NMDA_Tau_decay'    : STN_SNR_tau_nmda, # 100 ms standard approx, NMDA at -20 could shift to the left e.g. -40 
               'NMDA_Vact'         :-20.0,
               'NMDA_Sact'         : 16.0,
    ...
    model_list.append(('my_iaf_cond_exp', 'SNR_iaf', params))
...
def network(model_dict, Params_in={}):
    ...
    #! Configurable parameters
    #! =======================
    params = {'neurons':{'MSN_D1':{'type':'MSN_D1_spk_gen',
                                   'n':1500.0,
                                   'lesion':False},
    ...
    pos, layer_list,connect_list,  dist,f, tau={},[],[],{},{},{}
    for source in params['neurons'].keys():
        ...
        if not params['neurons'][source]['lesion']:
           layer_list.append((source, model_dict[source_type],
                                       { 'positions' : pos[source] , 
