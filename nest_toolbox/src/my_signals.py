...
class MyVmList(VmList):
    ...
    def my_set_spike_peak(self, peak, spkSignal):
        for id in self.ids:
            spikes=numpy.round((spkSignal[id].spike_times))
            spike=numpy.array(spikes, int)
            self.analog_signals[id].signal[numpy.array((self.t_start+spikes-2)/self.dt, int)] =peak
...
class MySpikeList(SpikeList):
    ...
    def binned_raw_data(self, t_start, t_stop, res=1, clip=1):
    ...
    def my_image_spike_histogram(sself, bin=10, display=True, id_list=[], n_rep=1, normalized=True, kwargs = {} ):
        '''
