...
#ifndef TSODYKS_BETA_CONNECTION_H
#define TSODYKS_BETA_CONNECTION_H
...
class TsodyksConnection : public nest::ConnectionHetWD
{
 public:
 ...
 private:
  nest::double_t tau_psc_rise_;    //!< [ms] time constant of postsyn current
  nest::double_t tau_psc_decay_;   //!< [ms] time constant of postsyn current
 ...
 */
inline
void TsodyksConnection::send(nest::Event& e, nest::double_t t_lastspike, const nest::CommonSynapseProperties &)
{
  ...
  // From Diesmann 1999
	// (1) exact solution exponential decay:    n(t) = a*e^(-at)
  // (2)      ""        beta shaped:          n(t) =(a+b)/ab*( e^(-at) - e^(-bt) )
	// (3) c = -a,        d = e^(-at):           n(t) = cd
	// (4) c = (a+b)/ab,  d = e^(-at) - e^(-bt): n(t) = cd



	// OLD
	//double_t Pyy = std::exp(-h/tau_psc_) = c from (3)

	// NEW
	// change to c = e^(-at) - e^(-bt) from (4)
	// with a =1/tau_psc_rise_, b=1/tau_psc_decay_in (4)
	nest::double_t Pyy = ( std::exp(-h/tau_psc_rise_) - std::exp(-h/tau_psc_decay_) );
  ...
  //OLD
	// nest::double_t Pxy = ((Pzz - 1.0)*tau_rec_ - (Pyy - 1.0)*tau_psc_) / (tau_psc_ - tau_rec_);
 	...
 	// NEW
	// change d = a = 1/tau_psc_ in (1) -> d = 1/(tau_rise + tau_decay)tau_rise*tau_decay) =
  // tau_psc_rise_*tau_psc_decay_/(tau_psc_rise_ + tau_psc_decay_) in (3)

	nest::double_t tau_psc_ = tau_psc_rise_*tau_psc_decay_/(tau_psc_rise_ - tau_psc_decay_);
	nest::double_t Pxy = ((Pzz - 1.0)*tau_rec_ - (Pyy - 1.0)*tau_psc_) / (tau_psc_ - tau_rec_);
...
#endif // TSODYKS_BETA_CONNECTION_H
