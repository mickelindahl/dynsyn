...
u'=a( b( V - V_r )^p - u + u_const )
 If v > V_peak then V = c + u*kc and u = u + d
...
kc         double - Slope of voltage reset point
...
GABAA_1_g_peak     double - GABAA 1 peak conductance in nS.
GABAA_1_Tau_rise   double - Rise time of the GABAA synaptic beta function in ms.
GABAA_1_Tau_decay  double - Decay time of the GABAA synaptic beta function in ms.

GABAA_2_E_rev      double - GABAA 2 reversal potential in mV.
GABAA_2_g_peak     double - GABAA 2 peak conductance in nS.
GABAA_2_Tau_rise   double - Rise time of the GABAA synaptic beta function in ms.
GABAA_2_Tau_decay  double - Decay time of the GABAA synaptic beta function in ms.
...
private:
	...
	struct Parameters_ {
		...
		nest::double_t kc;				 //!< Dlope of voltage reset poin
		...
		nest::double_t d_1;			   //!< Slow variable change at spike when u > du
		nest::double_t d_2;			   //!< Slow variable change at spike when u < du
		nest::double_t d_u;        //!< if u <du then d = d1 else d=d2
		nest::double_t d_1_bound;  //!< If 1 then d=d1 if -( u - du ) <d1 else d =-( u - du )
