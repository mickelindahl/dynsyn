...
{ 
	...
	// Total input current from synapses and external input. CURR is external
	// current input and I_e is constant current input.
	node.S_.I_ = I_AMPA + I_NMDA + I_GABAA_1 + I_GABAA_2 ;

	// Neuron dynamics
	// dV_m/dt
	f[0]= ( node.P_.k*( V - node.P_.V_r )*(  V - node.P_.V_t ) - u + node.S_.I_ + node.V_.CURR + node.P_.I_e )/node.P_.C_m;
	...
	f[2] = -y[S::DG_AMPA] / node.P_.AMPA_Tau_rise;
	f[3] =  y[S::DG_AMPA] - y[S::G_AMPA]/node.P_.AMPA_Tau_decay;

	// d dg_NMDA/dt, dg_NMDA/dt
	f[4] = -y[S::DG_NMDA] / node.P_.NMDA_Tau_rise;
	f[5] =  y[S::DG_NMDA] - y[S::G_NMDA]/node.P_.NMDA_Tau_decay;

	// d dg_GABAA/dt, dg_GABAA/dt
	f[6] = -y[S::DG_GABAA_1] / node.P_.GABAA_1_Tau_rise;
	f[7] =  y[S::DG_GABAA_1] - y[S::G_GABAA_1]/node.P_.GABAA_1_Tau_decay;

	// d dg_GABAA/dt, dg_GABAA/dt
	f[8] = -y[S::DG_GABAA_2] / node.P_.GABAA_2_Tau_rise;
	f[9] =  y[S::DG_GABAA_2] - y[S::G_GABAA_2]/node.P_.GABAA_2_Tau_decay;
...
void mynest::izhik_cond_beta::Parameters_::set(const DictionaryDatum& dd)
{
	...
	if ( AMPA_Tau_rise    <= 0 || AMPA_Tau_decay    <= 0 ||
			NMDA_Tau_rise    <= 0 || NMDA_Tau_decay    <= 0 ||
			GABAA_1_Tau_rise <= 0 || GABAA_1_Tau_decay <= 0 ||
			GABAA_2_Tau_rise <= 0 || GABAA_2_Tau_decay <= 0 )
		throw BadProperty("All time constants must be strictly positive.");


	if ( AMPA_Tau_rise >= AMPA_Tau_decay || NMDA_Tau_rise >= NMDA_Tau_decay )
		throw BadProperty("{AMPA, NMDA}_Tau_rise must greater than {AMPA, NMDA}_Tau_decay.");

	if ( GABAA_1_Tau_rise >= GABAA_1_Tau_decay || GABAA_2_Tau_rise >= GABAA_2_Tau_decay )
		throw BadProperty("{GABAA_1, GABAA_2}_Tau_rise must greater than {GABAA_1, GABAA_2}_Tau_decay.");
}
...
void mynest::izhik_cond_beta::init_buffers_()
{
	...
	B_.logger_.reset();  //As in hp_neuron, use to be init but now init is called in calibrate
...
void mynest::izhik_cond_beta::calibrate()
{
	...
	V_.cond_steps_.resize(MAX_SPIKE_RECEPTOR-1);
...
void mynest::izhik_cond_beta::update(Time const & origin, const nest::long_t from, const nest::long_t to)
{
	...
	for ( nest::long_t lag = from ; lag < to ; ++lag )
	{
		...
		// If threshold is crossed a spike is considered to have been generated
		...
		V_.CURR = B_.currents_.get_value(lag);
...
void mynest::izhik_cond_beta::handle(CurrentEvent& e)
{
	...
	// Assert that port is 0 (CURR - MAX_SPIKE_RECEPTOR = 0)
	assert(0 <= e.get_rport() && e.get_rport() == CURR - MAX_SPIKE_RECEPTOR);
