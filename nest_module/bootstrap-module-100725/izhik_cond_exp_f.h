...
#ifndef IZHIK_COND_EXP_F_H
#define IZHIK_COND_EXP_F_H
...
/* BeginDocumentation
Name: izhik_cond_exp_f - Izhikevich simple neuron

Description:
izhik_cond_exp_f is an implementation of based on  Izhikevich simple model
 ...
 */
extern "C"
int izhik_cond_exp_f_dynamics (double, const double*, double*, void*);
 ...
 *       It would be better to use receptor_types, cf izhik_cond_exp_f_mc.
 */
class izhik_cond_exp_f : public nest::Archiving_Node
{
...
public:
	...
	izhik_cond_exp_f();
	izhik_cond_exp_f(const izhik_cond_exp_f&);
	~izhik_cond_exp_f();
...
private:
	...
	//in namespace mynest.
	friend int mynest::izhik_cond_exp_f_dynamics(double, const double*, double*, void*);


	// The next two classes need to be friends to access the State_ class/member
	// Add nest:: since these are nest classes in namespace nest
	friend class nest::RecordablesMap<izhik_cond_exp_f>;
	friend class nest::UniversalDataLogger<izhik_cond_exp_f>;
...
private:
	 ...
	 */
	struct Buffers_ {
		Buffers_(izhik_cond_exp_f&); //!<Sets buffer pointers to 0
		Buffers_(const Buffers_&, izhik_cond_exp_f&); //!<Sets buffer pointers to 0

		//! Logger for all analog data
		nest::UniversalDataLogger<izhik_cond_exp_f> logger_;
	...
	static nest::RecordablesMap<izhik_cond_exp_f> recordablesMap_;
};
...
inline
nest::port mynest::izhik_cond_exp_f::check_connection(nest::Connection& c, nest::port receptor_type)
{
	nest::SpikeEvent e;
	e.set_sender(*this);
	c.check_event(e);
...
inline
nest::port mynest::izhik_cond_exp_f::connect_sender(nest::SpikeEvent&, nest::port receptor_type)
{
...
inline
nest::port mynest::izhik_cond_exp_f::connect_sender(nest::CurrentEvent&, nest::port receptor_type)
{
...
inline
nest::port mynest::izhik_cond_exp_f::connect_sender(nest::DataLoggingRequest& dlr,
...
inline
void izhik_cond_exp_f::get_status(DictionaryDatum &d) const
{
	P_.get(d);
	S_.get(d);
...
inline
void izhik_cond_exp_f::set_status(const DictionaryDatum &d)
{
